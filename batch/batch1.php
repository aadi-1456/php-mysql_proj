...
function drawChart() {
  ...
  <?php
             $re="btw 40 and 50";
             $sql="SELECT COUNT(*) AS number FROM `students` where batch='1' and dbms BETWEEN 41 AND 50";
             $result=mysqli_query($mysql_db,$sql);
             while($row=mysqli_fetch_assoc($result)){
                echo "['".$re."',".$row["number"]."],";
            }
            $re="btw 30 and 40";
            $sql="SELECT COUNT(*) AS number FROM `students` where batch='1' and dbms BETWEEN 31 AND 40";
            $result=mysqli_query($mysql_db,$sql);
            while($row=mysqli_fetch_assoc($result)){
                echo "['".$re."',".$row["number"]."],";
            }
            $re="btw 20 and 30";
            $sql="SELECT COUNT(*) AS number FROM `students` where batch='1' and dbms BETWEEN 21 AND 30";
            $result=mysqli_query($mysql_db,$sql);
            while($row=mysqli_fetch_assoc($result)){
                echo "['".$re."',".$row["number"]."],";
            }
            $re="btw 10 and 20";
            $sql="SELECT COUNT(*) AS number FROM `students` where batch='1' and dbms BETWEEN 11 AND 20";
            $result=mysqli_query($mysql_db,$sql);
            while($row=mysqli_fetch_assoc($result)){
                echo "['".$re."',".$row["number"]."],";
            }
            $re="btw 0 and 10";
            $sql="SELECT COUNT(*) AS number FROM `students` where batch='1' and dbms BETWEEN 1 AND 10";
            $result=mysqli_query($mysql_db,$sql);
            while($row=mysqli_fetch_assoc($result)){
                echo "['".$re."',".$row["number"]."],";
            }
...
function drawChart() {
  ...
  <?php
             $re="btw 40 and 50";
             $sql="SELECT COUNT(*) AS number FROM `students` where batch='1' and mp BETWEEN 41 AND 50";
             $result=mysqli_query($mysql_db,$sql);
             while($row=mysqli_fetch_assoc($result)){
                echo "['".$re."',".$row["number"]."],";
            }
            $re="btw 30 and 40";
            $sql="SELECT COUNT(*) AS number FROM `students` where batch='1' and mp BETWEEN 31 AND 40";
            $result=mysqli_query($mysql_db,$sql);
            while($row=mysqli_fetch_assoc($result)){
                echo "['".$re."',".$row["number"]."],";
            }
            $re="btw 20 and 30";
            $sql="SELECT COUNT(*) AS number FROM `students` where batch='1' and mp BETWEEN 21 AND 30";
            $result=mysqli_query($mysql_db,$sql);
            while($row=mysqli_fetch_assoc($result)){
                echo "['".$re."',".$row["number"]."],";
            }
            $re="btw 10 and 20";
            $sql="SELECT COUNT(*) AS number FROM `students` where batch='1' and mp BETWEEN 11 AND 20";
            $result=mysqli_query($mysql_db,$sql);
            while($row=mysqli_fetch_assoc($result)){
                echo "['".$re."',".$row["number"]."],";
            }
            $re="btw 0 and 10";
            $sql="SELECT COUNT(*) AS number FROM `students` where batch='1' and mp BETWEEN 1 AND 10";
            $result=mysqli_query($mysql_db,$sql);
            while($row=mysqli_fetch_assoc($result)){
                echo "['".$re."',".$row["number"]."],";
            }
...
function drawChart() {
  ...
  <?php
             $re="btw 40 and 50";
             $sql="SELECT COUNT(*) AS number FROM `students` where batch='1' and ss BETWEEN 41 AND 50";
             $result=mysqli_query($mysql_db,$sql);
             while($row=mysqli_fetch_assoc($result)){
                echo "['".$re."',".$row["number"]."],";
            }
            $re="btw 30 and 40";
            $sql="SELECT COUNT(*) AS number FROM `students` where batch='1' and ss BETWEEN 31 AND 40";
            $result=mysqli_query($mysql_db,$sql);
            while($row=mysqli_fetch_assoc($result)){
                echo "['".$re."',".$row["number"]."],";
            }
            $re="btw 20 and 30";
            $sql="SELECT COUNT(*) AS number FROM `students` where batch='1' and ss BETWEEN 21 AND 30";
            $result=mysqli_query($mysql_db,$sql);
            while($row=mysqli_fetch_assoc($result)){
                echo "['".$re."',".$row["number"]."],";
            }
            $re="btw 10 and 20";
            $sql="SELECT COUNT(*) AS number FROM `students` where batch='1' and ss BETWEEN 11 AND 20";
            $result=mysqli_query($mysql_db,$sql);
            while($row=mysqli_fetch_assoc($result)){
                echo "['".$re."',".$row["number"]."],";
            }
            $re="btw 0 and 10";
            $sql="SELECT COUNT(*) AS number FROM `students` where batch='1' and ss BETWEEN 1 AND 10";
            $result=mysqli_query($mysql_db,$sql);
            while($row=mysqli_fetch_assoc($result)){
                echo "['".$re."',".$row["number"]."],";
            }
  ...
  <div class="p-4">
    <div class="welcome">
      <div class="content rounded-3 p-3">
        ...
        <p class="mb-0">Hello <?php echo $_SESSION['username']; ?>, welcome to Batch B1 dashboard!</p>
      </div>
    </div>
      ...
      <div class="row">
        ...
        <div class="col-md-6 col-lg-3 mb-4 mb-lg-0">
          <div class="box bg-success p-3">
            <i class="uil-user"></i>
            <?php
                $sql = "SELECT batch FROM users WHERE batch='1'";
                $result=mysqli_query($mysql_db,$sql);
                if($result){
                    $row=mysqli_num_rows($result);
                }
                ?>
            <h3><?php echo $row ?></h3>
            <p class="lead text-bold">B1 Users</p>
          </div>
        </div>
      </div>
    </section>
